...
# в интерфейса их не выберут. Строковое значение у угроз поставил, потому что tkinter классы ругались на int-овый тип
...
def create_list_for_randchoice(ttk_list):
    result_of_query = cursor.execute(
        sqlselect(ttk_list)).fetchall()  # собираю все значения sql отбора и отображаю их все
...
def start():
    ...
    # На опустошенном мире нет строгих боевых действий, боевые конфликты уникальные и не будут с тегом боевого действия
        ...
        checkbuttons()  # Чекбоксы добавляются в самом конце после START-а, остальной словарь заполняется до этого
